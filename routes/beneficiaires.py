...
@router.post("/")
def create_beneficiary_route(body: Beneficiary, session: Session = Depends(get_session), get_user= Depends(get_user)):
    return create_beneficiary_service(body, session=session, get_user=get_user)

@router.get("/{uid}")
def get_beneficiaries_route( uid:str, session: Session = Depends(get_session),get_user= Depends(get_user),):
    return get_all_beneficiary(uid, session=session,get_user=get_user)
